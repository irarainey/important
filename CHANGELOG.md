...
- **Fix: Multiple aliased from-imports never merged**: Multiple aliased from-imports from the same module (e.g. `from X import a as x` and `from X import b as y`) are now kept as separate statements, matching Ruff/isort behaviour. Previously the deduplicator merged them into a single line (`from X import a as x, b as y`), which Ruff would then split back, creating a fix→Ruff→fix cycle.
...
- **Fix: Auto-fix for `unnecessary-from-alias`**: The `unnecessary-from-alias` rule now has an auto-fix. `from X import y as z` where the alias is unjustified is fixed by removing the `as` clause and replacing all references to the alias with the original name (e.g. `z.method()` → `y.method()`). Previously the rule only produced a diagnostic with no auto-fix, causing persistent unfixable violations after the first fix pass.
- **Fix: Improved from-alias fix matching**: The `unnecessary-from-alias` fixer now uses regex extraction to reliably identify the specific alias flagged in the issue message, with a fallback to substring matching. This improves robustness when fixing multiline imports with aliases.
